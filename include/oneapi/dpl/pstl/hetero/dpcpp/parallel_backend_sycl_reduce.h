...
class __reduce_single_wg_kernel;

template <typename... _Name>
class __reduce_multi_wg_main_kernel;

template <typename... _Name>
class __reduce_multi_wg_leaf_kernel;
                                                       ...
                                                       __internal::__optional_kernel_name<_Name...>>
{
                ...
                _InitType __init, std::true_type /*is_commutative*/, _Ranges&&... __rngs) const
    {
        ...
        _Size __n_items = ::std::min(__n, (_Size)__work_group_size); // number of work items

        sycl::event __reduce_event = __exec.queue().submit([&, __n, __n_items](sycl::handler& __cgh) {
            oneapi::dpl::__ranges::__require_access(__cgh, __rngs...); //get an access to data under SYCL buffer
            auto __res_acc = __res.template get_access<sycl::access_mode::discard_write>(__cgh);
                                                      ...
                                                      __internal::__optional_kernel_name<_LeafName...>>
{
                ...
                _InitType __init, std::true_type /*is_commutative*/, _Ranges&&... __rngs) const
    {
        ...
        sycl::event __reduce_event = __exec.queue().submit([&, __n, __n_items, __n_groups](sycl::handler& __cgh) {
            oneapi::dpl::__ranges::__require_access(__cgh, __rngs...); //get an access to data under SYCL buffer
            ...
            __cgh.parallel_for<_MainName...>(
                ...
                [=](sycl::nd_item<1> __item_id) {
                    ...
                    _Tp __result = __reduce_pattern(__item_id, __n_items, __temp_local);
                    if (__item_id.get_local_id(0) == 0)
                        __temp_acc[__item_id.get_group(0)] = __result;
                });
        });

        __n = __n_groups;
        __n_items = ::std::min(__n_groups, (_Size)__work_group_size);
        sycl::buffer<_Tp> __res(sycl::range<1>(1));
                            ...
                            _InitType __init, _Ranges&&... __rngs)
{
    ...
    // Use a single-pass tree reduction for small arrays.
